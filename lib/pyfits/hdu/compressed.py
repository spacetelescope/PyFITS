...
class CompImageHeader(Header):
    ...
    def update(self, key, value, comment=None, before=None, after=None,
        ...
        if (key not in ('SIMPLE', 'XTENSION', 'BITPIX', 'PCOUNT', 'GCOUNT',
...
class CompImageHDU(BinTableHDU):
    ...
    def updateHeaderData(self, image_header,
        ...
        for idx, axis in enumerate(self._axes):
            ...
            if image_header and naxis in image_header:
                self._header.update(
                    znaxis, axis, image_header.ascard[naxis].comment,
                    after=last_znaxis)
            else:
                self._header.update(znaxis, axis,
    ...
    def header(self):
        ...
        if 'GCOUNT' in self._image_header:
            ...
            after = None
        if 'BSCALE' in self._header:
            self._image_header.update('BSCALE', self._header['BSCALE'],
                                      self._header.comments['BSCALE'],
                                      after=after)
            after = 'BSCALE'

        if 'BZERO' in self._header:
            self._image_header.update('BZERO', self._header['BZERO'],
    ...
    def updateHeader(self):
        ...
        for idx in range(self.compData._nfields):
            if isinstance(self.compData._coldefs._recformats[idx], _FormatP):
                tform = 'TFORM' + str(idx + 1)
                key = self._header[tform]
                _max = self.compData.field(idx)._max
                if '(' in key:
                    key = key[:key.index('(')]
                self._header[tform] = '%s(%s)' % (key, _max)
