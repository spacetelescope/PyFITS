...
class _ImageBaseHDU(_ValidHDU):
    ...
    def __init__(self, data=None, header=None, do_not_scale_image_data=False,
        ...
        if data is DELAYED:
            ...
            if isinstance(self, ExtensionHDU):
                c0 = Card('XTENSION', 'IMAGE',
                          self.standard_keyword_comments['XTENSION'])
            else:
                c0 = Card('SIMPLE', True,
                          self.standard_keyword_comments['SIMPLE'])
            _list = CardList([
                c0,
                Card('BITPIX',    8, self.standard_keyword_comments['BITPIX']),
                Card('NAXIS',     0, self.standard_keyword_comments['NAXIS'])])

            if isinstance(self, GroupsHDU):
                _list.append(Card('GROUPS', True,
                                  self.standard_keyword_comments['GROUPS']))

            if isinstance(self, (ExtensionHDU, GroupsHDU)):
                _list.append(Card('PCOUNT',    0,
                                  self.standard_keyword_comments['PCOUNT']))
                _list.append(Card('GCOUNT',    1,
    ...
    header = property(header, _set_header)
    ...
    def update_header(self):
        ...
        if (not self._modified and not self._header._mod and
        ...
        if 'BITPIX' not in self._header:
            ...
            bitpix_comment = self._header.ascard['BITPIX'].comment

        # Update the BITPIX keyword and ensure it's in the correct
        # location in the header
        self._header.update('BITPIX', self._bitpix, bitpix_comment, after=0)
        ...
        if 'NAXIS' in self._header:
            naxis_comment = self._header.ascard['NAXIS'].comment
        else:
            naxis_comment = self.standard_keyword_comments['NAXIS']
        self._header.update('NAXIS', len(self._axes), naxis_comment,
                            after='BITPIX')
    ...
    def update_header(self):
        ...
        if 'EXTEND' in self._header:
            if len(self._axes):
                ...
                after = 'NAXIS'
            self._header.update('EXTEND', self._header['EXTEND'], after=after)
