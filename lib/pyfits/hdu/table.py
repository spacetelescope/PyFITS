...
class BinTableHDU(_TableBaseHDU):
    ...
    def _dump_data(self, fileobj):
        ...
        linewriter = csv.writer(fileobj, delimiter=' ', skipinitialspace=True)
        ...
        def format_value(val, format):
            if format[0] == 'S':
                ...
                return '%#21.15g+%#21.15gi' % (val.real, val.imag)
    ...
    def _load_data(cls, fileobj, coldefs=None):
        ...
        linereader = csv.reader(fileobj, delimiter=' ', skipinitialspace=True)
        ...
        for row in linereader:
            ...
            if coldefs is not None:
                continue
            # TODO: Here we look at the data in the row and update the formats
            # list accordingly...

        dtype = np.rec.format_parser(recformats, names, None).dtype

        # This is the recarray that will hold all the data from the file;
        # obviously we need enough physical memory to hold it
        ...
        data = np.recarray(shape=nrows, dtype=dtype)
    ...
    def _load_coldefs(cls, fileobj):
        ...
        for line in fileobj:
            words = line[:-1].split()
            for attr in ['names', 'formats', 'disps', 'units', 'dims']:
                getattr(coldefs, attr).append(words.pop(0).replace('""', ''))

            for attr in ['nulls', 'bscales', 'bzeros']:
                word = words.pop(0).replace('""', '')
                if word:
                    word = _str_to_num(word)
                getattr(coldefs, attr).append(word)
