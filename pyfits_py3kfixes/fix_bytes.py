...
def shouldtransform(node):
    ...
    sgparent = str(gparent)

    ggparent = gparent.parent
    sggparent = str(ggparent)

    gggparent = ggparent.parent
    sgggparent = str(gggparent)

#    if 'bool' in node.value:
#        import pdb; pdb.set_trace()

    if gparent.type in (syms.power, syms.return_stmt) and \
       'group' in sgparent:
        return False
    elif ggparent.type in (syms.power, syms.return_stmt) and (
       'getattr' in sggparent or 'hasattr' in sggparent or
       'setattr' in sggparent or 'encode' in sggparent or
       'decode' in sggparent or 'filterwarnings' in sggparent or
       'os.path.join' in sggparent):
        return False
    elif gggparent and '__doc__' in sgggparent:
        return False
    elif ggparent.type == syms.expr_stmt and \
       ggparent.children[0] in (Name('__all__'), Name('__version__')):
        return False
    elif gggparent and gggparent.type == syms.expr_stmt and \
         gggparent.children[0] == Name('__all__'):
        return False
